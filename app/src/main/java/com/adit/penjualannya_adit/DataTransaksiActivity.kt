...
class DataTransaksiActivity : AppCompatActivity() {
    ...
    private val listProduk = mutableListOf<ModelProdukTK>()
    ...
    private fun loadKategori() {
        db.getReference("kategori").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                chipSemua.setOnClickListener { selectedKategoriId = "Semua"; filterProduk() }
    ...
    private fun loadProduk() {
        db.getReference("produk").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                listProduk.clear()
                for (snap in snapshot.children) {
                    val produk = snap.getValue(ModelProdukTK::class.java)
    ...
    private fun setupSearch() {
        svProduk.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String?): Boolean { filterProduk(); return true }
        })
    }
    ...
    private fun filterProduk() {
        ...
        val filtered = listProduk.filter { produk ->
            ...
            val matchSearch = produk.namaProduk?.lowercase()?.contains(query) == true ||
                    produk.barcode?.lowercase()?.contains(query) == true
            matchKategori && matchSearch
        }
        adapterProduk.updateData(filtered)
    }

    private fun addToCart(produk: ModelProdukTK) {
        val existing = listCart.find { it.produk?.idProduk == produk.idProduk }
        if (existing != null) existing.jumlah++
        else listCart.add(ModelCartItem(produk, 1))
    ...
    private fun setupActions() {
        btnHapus.setOnClickListener {
            listCart.clear(); adapterCart.notifyDataSetChanged(); updateSummary()
        }
        btnBayar.setOnClickListener {
            if (listCart.isEmpty()) { Toast.makeText(this, "Keranjang kosong", Toast.LENGTH_SHORT).show(); return@setOnClickListener }
            saveTransaction()
        }
    }
    ...
    private fun saveTransaction() {
        ...
        var subtotal = 0
        for (item in listCart) subtotal += (item.produk?.hargaJual ?: 0) * item.jumlah

        val date = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date())
        val transaksi = ModelTransaksi(
            idTransaksi = id, listProduk = listCart.toList(),
            subtotal = subtotal, totalHarga = subtotal, diskon = 0,
            tanggal = date, namaPegawai = "Kasir"
        )
